...
public class ModuleTestBase extends TestCase {
	...
	protected void assertListSelectedConfiguration(String expectedTitle) {   
		...
		assertEquals(expectedTitle, refineComboTitle(title));
	}
	...
	private void assertAllComboOptions(HtmlSelect combo, String ... expectedTitles) throws Exception {   
		...
		for (HtmlOption option: combo.getOptions()) {
			String title = titles.isEmpty()?refineComboTitle(option.asText()):option.asText();  
			titles.add(title);
		}
	...
	private String refineComboTitle(String title) {
